...
7. **Remembering.** The episode is written *after* the decision it records,
   which is what makes :meth:`marketlab.memory.store.MemoryStore.recall`'s
   strictly-before cutoff sufficient: a cycle interrupted between the two and
   then resumed recalls its own decision only if the episode was written
   first, and here it never is.
...
implementation of it. A replay configures no recorder: memory exists to
influence decisions, and a replay makes none.
"""
...
from marketlab.experiments.materials import MemoryRecorder
...
class CycleDriver:
    ...
    """Writes each condition's episode and reflects on its cadence. Left unset
    for a replay, which makes no decisions to remember."""
    ...
    def remember(self, cycle: CycleResult, *, cycle_index: int) -> None:
        """Step 7. Written after the decision, never before it."""
    ...
    def run(self, *, cycle_index: int, snapshot_id: str, as_of: Instant) -> CycleReport:
        ...
        self.remember(cycle, cycle_index=cycle_index)
